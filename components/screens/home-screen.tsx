...
import { PageBefore } from "@/components/page-chrome"
import { usePersistedChoice, readPersistedChoice } from "@/hooks/use-persisted-choice"
...
/** useLayoutEffect warns during SSR. Same guard, and the same reason, as `components/mode-switch.tsx`. */
...
const deriveMeditationFileName = (meditation: any): string => {
  ...
  if (hasExtension) return derivedTitle

  const container = meditation?.metadata?.audioFormat?.container as AudioFormatMetadata["container"] | undefined
  return `${derivedTitle}.${extensionForContainer(container)}`
}

/**
 * The same snappy spring the mode switch's pill uses, for the same reason — the two switches are a
 * few hundred pixels apart and a user will see both in one glance, so they had better move alike.
 */
const TAB_PILL_SPRING = { type: "spring", stiffness: 420, damping: 40 } as const

/** Where the tab pill is, in the coordinates of the tab track it sits in. */
type TabPillBox = { x: number; y: number; w: number; h: number }
...
export function HomeScreen() {
  ...
  const [activeMode, setActiveMode] = usePersistedChoice("home-mode", TOOL_MODES, "adjuster")
  ...
  const [activeTab, setActiveTab] = usePersistedChoice("home-tab", TOOL_MODES, "adjuster")
  // Which of Settings / Advanced the Adjuster's own tabs are showing. See the Tabs below. A plain
  // string rather than `ToolMode`: it tracks Radix's value, which is the tab's, and the two are
  // only coincidentally the same union.
  const [shownTab, setShownTab] = useState<string>(activeTab)
  const tabsListRef = useRef<HTMLDivElement>(null)
  const [tabPill, setTabPill] = useState<TabPillBox | null>(null)

  // Re-measured whenever the shown tab changes, and never computed from the grid. The two columns
  // are equal today, but their width comes from the track's padding, the gap and whatever the
  // labels turn out to be; the pill has no business having an opinion about any of that.
  //
  // A layout effect, so the pill is already moving towards its new box in the same frame the
  // click's re-render commits — a passive effect would leave one frame with the pill still settled
  // on the tab that is no longer selected.
  useMeasureEffect(() => {
    ...
    const next: TabPillBox = { x: rect.x - track.x, y: rect.y - track.y, w: rect.width, h: rect.height }
    setTabPill((prev) =>
  ...
  //
  // The debouncer flushes on unmount rather than cancelling (hooks/use-debounced-save.ts). That is
  // not a detail: this effect's own cleanup runs on every edit, so the *last* 800ms of work — the
  // adjustment just made, right before swiping to the Library — was the one thing the old
  // hand-rolled version reliably threw away.
  const draftSaver = useDebouncedSave(saveCurrentToolDraft, 800)
  useEffect(() => {
    const hasContent = (activeMode === "adjuster" && !!file) || (activeMode === "creator" && timelineEvents.length > 0)
    if (!hasContent) {
      // Resetting the tool clears the stored draft, so a save still on the clock from before the
      // reset must be dropped rather than allowed to write it straight back.
      draftSaver.cancel()
      return
    }
    draftSaver.schedule()
  }, [draftSaver, saveCurrentToolDraft, activeMode, file, timelineEvents.length])
  ...
  const reconstructCreatorMeditation = useCallback(
    async (importData: any) => {
      ...
      try {
        ...
        const audioFile = new File([audioBlob], reconstructedFileName, { type: audioBlob.type || "audio/wav" })
        ...
        const audioFile = new File([audioBlob], fallbackFileName, { type: audioBlob.type || "audio/wav" })
        setFile(audioFile)
        setOriginalUrl(URL.createObjectURL(audioFile))
  ...
  const importAsRecordedBlock = useCallback(
    async (importData: any) => {
      ...
      try {
        ...
        const audioFile = new File([audioBlob], recordedBlockFileName, { type: audioBlob.type || "audio/wav" })
    ...
    async (importData: any, sourceTab: "adjuster" | "creator") => {
      ...
      const importIntoAdjuster = async (data: any) => {
        ...
        try {
          ...
          const audioFile = new File([audioBlob], importedFileName, { type: audioBlob.type || "audio/wav" })
  ...
  // The active mode is persisted by usePersistedChoice above — it used to be written here too,
  // and that second writer was a bug: this effect fired on mount with the default "adjuster" and
  // overwrote the stored value before the restore below could read it, so a reload always landed
  ...
  useEffect(() => {
    ...
    const checkForImports = () => {
      ...
      try {
        // Read synchronously rather than leaning on the mode state, so this agrees with where
        // usePersistedChoice lands no matter which effect runs first. The mode itself is already
        // restored by that hook, so there is nothing to set here — only the meditation that was
        // open in it to bring back.
        const lastMode = readPersistedChoice("home-mode", TOOL_MODES)
  ...
  // `toggleTimer` and the "which tool was I on before the Timer" ref that went with it now live
  // in components/page-chrome.tsx, next to the switch that presses them. They write `home-mode`
  // and `home-tab` — the same keys this page reads — so the content below still follows.
  ...
  return (
    <>
      {/* The card, the wash, the logo, the switch and the signed-out auth buttons are all rendered
          by the layout now (components/page-chrome.tsx) so that they survive navigation. What is
          left here is the page's own content, plus the two things that belong *outside* the card:
          a viewport-fixed debug button, and a warning banner that sits above it. */}
      <PageBefore>
            {isLocalDebugHost && (
              <div className="fixed bottom-4 left-4 z-50">
                <Button
                  ...
                  variant="outline"
                  size="sm"
                  className="bg-red-50 text-red-600 border-red-200 hover:bg-red-100 opacity-70 hover:opacity-100"
                >
                  Clear Library (Debug)
                </Button>
              </div>
            ...
            {/* pt-24 md:pt-14 is the Library's and the Journal's header padding — it clears the
                fixed navigation bar. Signed out, this page is the only one that still shows its
                ...
                <div ref={adjusterSectionRef} className="space-y-4">
                    ...
                    <div className="p-4 rounded-lg border-logo-rose-300 max-w-2xl mx-auto backdrop-blur-sm border-0 py-4 px-0 bg-transparent pt-0 pb-0">
                      ...
                      <div className="text-sm text-gray-600 leading-relaxed flex flex-wrap justify-center text-center gap-[5px] px-2">
                        <a
                          ...
                          className="font-black text-gray-600 px-5 py-1 transition-all duration-200 ease-out hover:shadow-none shadow-md border-gray-500 text-xs border-[3px] rounded-[8px] tracking-tight bg-white pt-[3px]"
                        >
                          Dharma Seed
                        </a>
                        <a
                  ...
                  >
                    <Tabs
                      ...
                      // A mirror of what the Tabs is actually showing, and nothing else. The two
                      // triggers deliberately disagree with `activeTab` — "Advanced" leaves it on
                      // "adjuster" — so the pill cannot be driven from `activeTab` without changing
                      // what that means. Radix already tracks its own selection; this is a
                      // subscription to it for the pill's benefit alone.
                      onValueChange={setShownTab}
                      className="w-full font-serif font-black"
                    >
                      <TabsList ref={tabsListRef} className="relative grid w-full grid-cols-2 gap-2 mb-6 rounded-sm">
                        {/* One pill, always mounted, moved by measurement — not one per trigger
                            with a shared `layoutId`. The shared-layout version was tried and has a
                            defect: Framer crossfades the outgoing element by painting a lead copy
                            of it over the trigger it left, which wipes the label underneath for
                            about four frames. One element has nothing to crossfade, and it is how
                            `components/mode-switch.tsx` already does this. It is also why the pill
                            is painted *here*, before the triggers: both are positioned, so the
                            triggers and their labels paint above it.

                            The trigger's own `data-[state=active]:bg-white` is switched off below —
                            two backgrounds for one selection is one too many, and only the pill can
                            travel. */}
                        {tabPill && (
                          <motion.div
                            aria-hidden
                            className="pointer-events-none absolute rounded-[10px] bg-white shadow-md"
                            initial={false}
                            animate={{ left: tabPill.x, top: tabPill.y, width: tabPill.w, height: tabPill.h }}
                        ...
                        <TabsTrigger
                          ...
                          className="relative rounded-[10px] data-[state=active]:bg-transparent data-[state=active]:shadow-none"
                        >
                          Settings
                        </TabsTrigger>
                        <TabsTrigger
                      ...
                      <div className="bg-gradient-to-br from-logo-blue-400 to-logo-amber-300 px-6 py-[9px] text-center pt-[9px]">
                        ...
                        <div className="px-6 pb-0 pt-[21px]">
                          <input
                            ...
                            className="flex w-full ring-offset-background file:border-0 file:bg-white file:text-xs file:font-medium file:text-foreground placeholder:text-gray-500 focus-visible:outline-none disabled:cursor-not-allowed md:text-xs rounded-[10px] bg-white py-4 px-4 text-xs font-black text-gray-600 inset-shadow-recess h-9 border-[#f2f2f2] border-[0px]"
                          />
                        </div>
                                  ...
                                  <div className="flex items-center gap-5">
                                    ...
                                    <div className="flex items-center gap-2">
                                      ...
                                      <button
                                        ...
                                        // Same trough as the Timer's toggles and the app's main
                                        // switches: --recess with an inset edge, and a shadow on
                                        // the thumb so it stays readable against it.
                                        className={`relative inline-flex h-4 w-8 shrink-0 items-center rounded-full shadow-inner transition-colors ${
                                          multiNoteMode ? "bg-gray-500" : "bg-recess"
                                        }`}
                                      >
                                        <span
