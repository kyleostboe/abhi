...
const Chart: React.FC<ChartProps> = ({ data, chartConfig, chartType, title, description, className }) => {
          ...
          <ResponsiveContainer width="100%" height={300}>
            <ChartComponent data={data}>
              ...
              {Object.entries(chartConfig).map(([key, config]) => {
                if (key === "month") return null // Skip month as it's for XAxis
                return (
                  <DataComponent
                    key={key}
                    dataKey={key}
                    type="monotone" // Default for line/area, ignored by bar
                    stroke={config.color}
                    fill={config.color}
                    dot={chartType === "line"}
