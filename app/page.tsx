...
const updateBackgroundSoundVolume = useCallback((id: string, volume: number) => {
  setBackgroundSounds((prev) => prev.map((s) => (s.id === id ? { ...s, volume: Math.max(0, Math.min(1, volume)) } : s)))
}, [])
          ...
          <Tabs value={activeMode} onValueChange={(value) => setActiveMode(value as "adjuster" | "labs")} className="w-full">
            ...
            <TabsContent value="adjuster">
              <Card className="p-6 space-y-6">
                ...
                {status && (
                  <Alert className={cn({
                    "bg-green-100 border-green-400 text-green-700 dark:bg-green-900 dark:border-green-600 dark:text-green-200": status.type === "success",
                    "bg-red-100 border-red-400 text-red-700 dark:bg-red-900 dark:border-red-600 dark:text-red-200": status.type === "error",
                    "bg-blue-100 border-blue-400 text-blue-700 dark:bg-blue-900 dark:border-blue-600 dark:text-blue-200": status.type === "info",
                    "bg-yellow-100 border-yellow-400 text-yellow-700 dark:bg-yellow-900 dark:border-yellow-600 dark:text-yellow-200": status.type === "warning",
                  })}>
                      ...
                      <AccordionItem value="settings">
                        ...
                        <AccordionContent className="space-y-4 p-2">
                          ...
                          <p className="text-sm text-gray-500 dark:text-gray-400">
                            When enabled, the tool will primarily adjust silence durations to meet the target length, preserving the speaker's natural pacing. When disabled, the entire audio will be stretched or compressed.
                          </p>
                          ...
                          <div className="space-y-2">
                            ...
                            >
                              ...
                              <SelectContent>
                                ...
                                <SelectItem value="low">Low (WebM/Opus - smaller file size, wider browser support)</SelectItem>
                              </SelectContent>
                            </Select>
                            <p className="text-sm text-gray-500 dark:text-gray-400">
                              High compatibility produces standard WAV files. Low compatibility uses WebM/Opus for smaller files, but may not be supported by all legacy audio players.
                            </p>
                          </div>
                        </AccordionContent>
                      </AccordionItem>
                ...
                {audioAnalysis && (
                  <div className="space-y-2 text-sm text-gray-600 dark:text-gray-300">
                    ...
                    {actualDuration !== null && (
                      <p>Actual Processed Duration: {formatTime(actualDuration * 1000)}</p>
                    )}
              ...
              <Card className="p-6 space-y-6">
                ...
                <div className="space-y-4">
                  ...
                  <VisualTimeline
                    ...
                    onEventDrop={handleTimelineEventDrop}
                    onEventDurationChange={handleTimelineEventDurationChange}
                    onEventStartTimeChange={handleTimelineEventStartTimeChange}
                    onEventDelete={deleteTimelineEvent}
                  />
                      ...
                      <Tabs value={currentTab} onValueChange={setCurrentTab}>
                        ...
                        <TabsContent value="instructions" className="mt-4 space-y-4">
                          <Select
                            ...
                            onValueChange={(id) =>
                              setSelectedLibraryInstruction(INSTRUCTIONS_LIBRARY.find((instr) => instr.id === id) || null)
                            }
                          >
                            <SelectTrigger>
                          ...
                          <Button onClick={addInstructionToTimeline} disabled={!selectedLibraryInstruction} className="w-full">
                        ...
                        <TabsContent value="custom-instruction" className="mt-4 space-y-4">
                          ...
                          <Textarea
                            ...
                            rows={4}
                          />
                          <Button onClick={addInstructionToTimeline} disabled={!customInstructionText.trim()} className="w-full">
                        ...
                        {AMBIENT_SOUNDS_LIBRARY.map((sound) => (
                          ...
                          >
                            ...
                            {currentPlayingBackgroundSoundId === sound.id ? (
                              <StopCircle className="h-4 w-4 ml-2" onClick={(e) => { e.stopPropagation(); stopBackgroundSoundPreview(); }} />
                            ) : (
                              <Play className="h-4 w-4 ml-2" onClick={(e) => { e.stopPropagation(); playBackgroundSoundPreview(sound); }} />
                            )}
                          </Button>
                        ))}
                      </div>
