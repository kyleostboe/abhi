...
import { MUSICAL_NOTES } from "@/lib/musical-notes"
...
export default function HomePage() {
  ...
  const [readyToAddToTimelineRecording, setReadyToAddToTimelineRecording] = useState<boolean>(false)
  ...
  const handleFileSelectAction = useCallback(
    async (event: React.ChangeEvent<HTMLInputElement>) => {
      ...
      if (selectedFile.size > maxSize) {
        toast({
          ...
          description: `Please select a file smaller than ${isMobileDevice ? "50MB" : "500MB"}.`,
          variant: "destructive",
        })
        return
      }
      ...
      try {
        ...
        const analysis = await analyzeAudio(audioBuffer, silenceThreshold, minSilenceDuration)
        setAudioAnalysis(analysis)

        const minDuration = Math.max(5, Math.ceil(analysis.contentDuration / 60))
        const maxDuration = isMobileDevice ? 60 : 120
        setDurationLimits({ min: minDuration, max: maxDuration })
        setTargetDuration(Math.max(minDuration, Math.min(targetDuration, maxDuration)))

        const estimatedMemoryUsage = audioBuffer.length * audioBuffer.numberOfChannels * 4
        const memoryThreshold = isMobileDevice ? 100 * 1024 * 1024 : 500 * 1024 * 1024
        setMemoryWarning(estimatedMemoryUsage > memoryThreshold)

        toast({
          title: "File uploaded successfully",
          description: `Analyzed ${formatTime(audioBuffer.duration)} of audio.`,
        })
      } catch (error) {
    ...
    [isMobileDevice, silenceThreshold, minSilenceDuration, targetDuration, toast],
  )

  const handleDragOverAction = useCallback((e: React.DragEvent) => {
    e.preventDefault()
    e.stopPropagation()
  }, [])

  const handleDragLeaveAction = useCallback((e: React.DragEvent) => {
    e.preventDefault()
    e.stopPropagation()
  }, [])
  ...
  const handleDropAction = useCallback(
    async (e: React.DragEvent) => {
      ...
      if (audioFile) {
        const event = { target: { files: [audioFile] } } as any
        await handleFileSelectAction(event)
      }
    },
    [handleFileSelectAction],
  )

  const processAudioAdjusterAction = useCallback(async () => {
    if (!originalBuffer || !durationLimits) return
    ...
    try {
      ...
      setProcessedUrl(url)
      setActualDuration(result.actualDuration)
      ...
      toast({
        ...
        description: `Adjusted ${result.pausesAdjusted} pauses to ${formatTime(result.actualDuration)}.`,
      })
    } catch (error) {
      console.error("Error processing audio:", error)
      toast({
        title: "Processing failed",
        description: "Please try adjusting the settings or using a different file.",
        variant: "destructive",
      })
    } finally {
  ...
  const downloadProcessedAudioAction = useCallback(() => {
    if (!processedUrl || !file) return

    const a = document.createElement("a")
    a.href = processedUrl
    a.download = `${file.name.replace(/\.[^/.]+$/, "")}_adjusted_${targetDuration}min.wav`
    document.body.appendChild(a)
    a.click()
    document.body.removeChild(a)
  }, [processedUrl, file, targetDuration])
  ...
  const playSingleNote = async (note: string, octave: number, instrument: string) => {
    try {
      ...
      if (instrument === "harp") {
        audioBuffer = await generateHarpNote(note, octave)
      } else if (instrument === "piano") {
        audioBuffer = await generatePianoNote(note, octave)
      } else {
        audioBuffer = await generateSineWave(note, octave, 1.0)
      }
  ...
  const playChordPreview = async () => {
    ...
    try {
      ...
      for (const noteString of selectedNotes) {
  ...
  const timelinePlaySingleNote = async (note: string, octave: number, instrument: string) => {
    await playSingleNote(note, octave, instrument)
  }

  const timelinePlayChordPreview = async (notes: string[], instrument: string) => {
    try {
      const audioContext = new AudioContext()
      const masterGain = audioContext.createGain()
      masterGain.gain.value = 0.3
  ...
  const handleAddInstructionSoundEvent = () => {
    ...
    const newEvent = {
      ...
      type: multiNoteMode && selectedNotes.length > 1 ? "chord" : "instruction_sound",
      startTime: 0,
      instruction: customInstructionText.trim(),
      soundCue: multiNoteMode ? null : selectedSoundCue,
      notes: multiNoteMode ? selectedNotes : [],
      instrument: noteType,
    }
  ...
  const handleDuplicateEvent = (event: any) => {
    ...
    addEventToTimeline(duplicatedEvent)
  }
  ...
  const HomeContent = () => (
    <div className="relative overflow-hidden">
      ...
      <div className="px-6 md:px-10 font-serif font-black pb-7">
        ...
        {activeMode === "adjuster" ? (
          // Adjuster UI - keeping all original functionality
          <>
            {memoryWarning && (
                ...
                <div className="flex items-start">
                  ...
                  <div>
                    ...
                    <p className="text-yellow-600 font-serif font-black text-xs">
                      Large files or long target durations require significant memory. Processing may be slow or
                      unstable on devices with limited RAM.
                    </p>
                  </div>
                </div>
              </div>
...
A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"
          ...
          // Encoder UI - keeping all original functionality
          <motion.div
            key="encoder-content"
    ...
    return (
      <div className="px-6 md:px-10 pb-10">
        <div className="text-center mb-8">
          <h2 className="text-3xl font-black text-gray-800 mb-4 font-serif">Library</h2>
