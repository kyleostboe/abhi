...
const handleAddInstructionSoundEvent = () => {
  ...
  // Calculate the start time based on existing events
  let startTime = 0
  if (timelineEvents.length === 0) {
    // First event goes at the beginning
    startTime = 0
  } else {
    // Calculate equally spaced positions for all events including the new one
    const totalEvents = timelineEvents.length + 1
    const spacing = labsTotalDuration / (totalEvents + 1) // +1 to avoid placing at the very end
    startTime = spacing * totalEvents // Position for the new event
  }

  const newEvent: TimelineEvent = {
    id: `event_${Date.now()}_${Math.random().toString(36).substring(2, 9)}`,
    type: "instruction_sound",
    startTime: startTime, // Assign the calculated start time
  ...
  toast({
    title: "Event Added",
    description: `"${instructionTextToAdd.substring(0, 30)}..." with ${selectedSoundCue.name} added at ${Math.floor(startTime / 60)}:${String(Math.floor(startTime % 60)).padStart(2, "0")}.`,
  })
}
                      ...
                      <div className="grid md:grid-cols-2 gap-6 text-logo-rose-600">
                        <div className="text-center">
                          ...
                          <Input
                            ...
                            className="mt-1 text-xs font-black text-logo-rose-600 shadow-inner border border-logo-rose-600 focus:ring-logo-rose-600 focus:border-logo-rose-600"
                          />
                        </div>
                        <div className="text-center">
                  ...
                  >
                    <Card className="overflow-hidden border-none shadow-lg dark:shadow-white/20 bg-white dark:bg-gray-900 h-full">
                      ...
                      <div className="p-6 space-y-4 font-black">
                        ...
                        <Button
                          className="w-full bg-white text-logo-emerald-700 border border-logo-emerald-600 hover:bg-gray-50 dark:bg-gray-900 dark:text-logo-emerald-400 dark:border-logo-emerald-700 dark:hover:bg-gray-800"
                              ...
                              <Button
                                onClick={async () => {
                                  ...
                                  toast({
                                    ...
                                    description: `"${recordingLabel.trim()}" added to timeline at ${Math.floor(startTime / 60)}:${String(Math.floor(startTime % 60)).padStart(2, '0')}.`,
                                  })
                                }}
